      ...
      <div className="shrink-0 border-b border-border glass elev z-20">
        ...
        <div className="px-3 sm:px-5 py-2.5 flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <a href="/cas.html" target="_blank" rel="noopener" title="Closing Auction (CAS) Predictor — where the option chain is pulling the 15:30 close"
                ...
                <div className="card elev data-panel">
                  <div className="px-4 sm:px-5 py-3.5 text-[11px] mono text-txt-sec border-b border-border flex items-center gap-3 flex-wrap rounded-t-2xl" style={{ background: tint(active.color, 0.06) }}>
                    ...
                    {genTR && genTR.decided >= 20 && (genTR.winRate < 45
                      ? <span className="pill text-[10px] px-2 py-0.5 font-bold" style={{ background: 'color-mix(in srgb, var(--color-red) 16%, transparent)', color: 'var(--color-red)' }} title="Below 45% — still traded, but under close review: every loss gets a post-mortem and the fix is applied. Not benched.">⚠ under review · learning</span>
                      : genTR.winRate >= 75
                        ? <span className="pill text-[10px] px-2 py-0.5 font-bold" style={{ background: 'color-mix(in srgb, var(--color-green) 16%, transparent)', color: 'var(--color-green)' }} title="≥75% on a real closed sample → proven">✓ proven</span>
                        : <span className="pill text-[10px] px-2 py-0.5 font-bold" style={{ background: 'color-mix(in srgb, var(--color-yellow) 16%, transparent)', color: 'var(--color-yellow)' }} title="Working toward the 75% target — learning from each loss (see the post-mortem journal), not suppressed.">⚙ improving → 75%</span>)}
